...
def materiais_novasp(
    tb_materiais: win32.CDispatch,
    num_material_linhas: int,
    lacre: bool,
    estoque: DataFrame,
    ...
    for n_material in range(num_material_linhas):
        ...
        if sap_material in tb_contratada:
            # Marca Contratada
            tb_materiais.modifyCheckbox(n_material, "CONTRATADA", True)
            continue
        ...
        if lacre is False:
            if sap_material in ("50000328", "50000263"):
                ...
                lacre_material.caca_lacre(tb_materiais, sap_etapa_material, estoque, session)
                lacre = True
...
def materiais_gb_itaquera(
    tb_materiais: win32.CDispatch,
    num_material_linhas: int,
    lacre: bool,
    ultima_linha_material: int,
    estoque: DataFrame,
    session: win32.CDispatch,
) -> None:
    """Contratada GB - MLN."""
...
def materiais_contratada(tb_materiais: win32.CDispatch, contrato: str, estoque: DataFrame, session: win32.CDispatch) -> None:
    ...
    match contrato:
        case "4600041302":
            materiais_novasp(tb_materiais, num_material_linhas, lacre, estoque, session)
        case "4600042975":
            materiais_novasp(tb_materiais, num_material_linhas, lacre, estoque, session)
        case "4600042888":
            materiais_gb_itaquera(tb_materiais, num_material_linhas, lacre, ultima_linha_material, estoque, session)
        case "4600056089":
            materiais_gb_itaquera(tb_materiais, num_material_linhas, lacre, ultima_linha_material, estoque, session)
        case _:
            return
